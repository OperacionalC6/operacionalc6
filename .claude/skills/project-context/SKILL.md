...
- **Frontend**: ainda não existe. Decidido: Next.js, hospedado na Vercel.
- **Hospedagem backend+worker RPA+Postgres**: Render (motivo: sem time técnico interno além do usuário +
  Claude, Render é mais simples de manter que AWS/GCP "puro"; e o RPA precisa de disco persistente pro
  perfil do navegador, o que exclui opções serverless puras).
...
- **Contas de nuvem**: TUDO deve ser criado com conta/e-mail da empresa, nunca a conta pessoal do usuário
  nem rodando na máquina dele. Isso ainda não foi feito (ver "Próximos passos").
...
- Criação das contas de nuvem da empresa (Render, Vercel)
